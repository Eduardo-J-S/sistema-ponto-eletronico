...
function EditarHorarios() {
    ...
    const { buscarHorarios, atualizarHorario, deletarHorario, AdicionarHorario } = useContext(PerfilContext);
    ...
    const salvarHorario = async (index) => {
        try {
            ...
            if (horario.isNew) {
                ...
                const response = await atualizarHorario(horario.id, horario.diaSemana, horario.horaInicio, horario.horaFim, funcionario.id);
                if (response.success) {
                            ...
                            <div className="horario-info">
                                {horario.isNew ? (
                                    <select 
                                        ...
                                        onChange={(e) => handleEdit(index, 'diaSemana', e.target.value)}
                                    >
                                ...
                                <input 
                                    ...
                                    onChange={(e) => handleEdit(index, 'horaInicio', e.target.value)}
                                />
                                <input 
                                    type="time" 
                                    value={horario.horaFim} 
                                    onChange={(e) => handleEdit(index, 'horaFim', e.target.value)}
                                />
                            </div>
                            <div className="actions">
                                <button className="edit-btn" onClick={() => salvarHorario(index)}>Salvar</button>
                                <button className="delete-btn" onClick={() => deletarHorarioFuncionario(index)}>Excluir</button>
                            </div>
                        </div>
                    ))}
                    <button disabled={isFuncionarioAdded} className="add-btn" onClick={adicionarHorario}>Adicionar Novo Horário</button>
                </div>
            ) : (
                <div>
                    <p className="no-horarios">Nenhum horário encontrado para este CPF.</p>
                    <button className="add-btn" onClick={adicionarHorario}>Adicionar Novo Horário</button>
                </div>
            )}
        </div>
    );
