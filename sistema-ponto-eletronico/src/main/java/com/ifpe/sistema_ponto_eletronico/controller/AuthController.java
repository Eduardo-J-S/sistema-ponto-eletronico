...
public class AuthController {
    ...
    public ResponseEntity login(@RequestBody FuncionarioLoginRequestDTO body) {
        ...
        if (passwordEncoder.matches(body.senha(), funcionario.getSenha())) {
            ...
            return ResponseEntity.ok(new FuncionarioLoginResponseDTO(funcionario.getId(), funcionario.getNome(), funcionario.getMatricula(), funcionario.getCpf(), funcionario.getStatus(), funcionario.getRole().getRole(), funcionario.getEmpresa().getId(), token));
        }
    ...
    public ResponseEntity register(@RequestBody EmpresaRequestDTO body) {
        ...
        if (func.isEmpty()) {
            ...
            return ResponseEntity.ok(new FuncionarioLoginResponseDTO(admin.getId(), admin.getNome(), admin.getMatricula(), admin.getCpf(), admin.getStatus(), admin.getRole().getRole(), admin.getEmpresa().getId(), token));
        }
        return ResponseEntity.badRequest().build();
    }
}
