...
public class AuthController {
    ...
    public ResponseEntity login(@RequestBody FuncionarioLoginRequestDTO body) {
        ...
        if (passwordEncoder.matches(body.senha(), funcionario.getSenha())) {
            ...
            return ResponseEntity.ok(new FuncionarioLoginResponseDTO(funcionario.getNome(), funcionario.getMatricula(), funcionario.getRole().getRole(), token));
        }
    ...
    public ResponseEntity register(@RequestBody EmpresaRequestDTO body) {
        ...
        if (func.isEmpty()) {
            ...
            return ResponseEntity.ok(new FuncionarioLoginResponseDTO(admin.getNome(), admin.getMatricula(), admin.getRole().getRole(), token));
        }
        return ResponseEntity.badRequest().build();
    }
}
